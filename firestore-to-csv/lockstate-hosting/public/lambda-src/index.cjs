...
/* Sensor redesign toggle – set Lambda env variable SENSOR_MODE=contact to switch */
...
function stateToSensor(state) {
  // DETECTED means closed
  return (state === 1 || state === 3) ? 'DETECTED' : 'NOT_DETECTED';
}

function buildStateProperty(state) {
  if (SENSOR_MODE) {
    return {
      namespace: 'Alexa.ContactSensor',
      name     : 'detectionState',
      value    : stateToSensor(state),
  ...
  if (SENSOR_MODE) {
    return [
      ...
      { type:'AlexaInterface', interface:'Alexa.ContactSensor', version:'3',
        properties:{ supported:[{name:'detectionState'}], proactivelyReported:true, retrievable:true } },
...
async function smartHome(event) {
  ...
  if (namespace === 'Alexa.Authorization' && name === 'AcceptGrant') {
    ...
    try {
      const grantCode     = payload.grant?.code;
      const granteeToken  = payload.grantee?.token; // UID we issued
      if (SENSOR_MODE && grantCode && granteeToken && process.env.ALEXA_CLIENT_ID && process.env.ALEXA_CLIENT_SECRET) {
        log('INFO', 'AcceptGrant exchanging code for refresh_token', { grantCode, granteeToken });
        ...
        const res = await fetch('https://api.amazon.com/auth/o2/token', { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body });
        if (!res.ok) {
          const txt = await res.text().catch(()=>'');
          log('ERROR', 'AcceptGrant token exchange failed', { status: res.status, txt });
        } else {
          const j = await res.json();
          log('INFO', 'AcceptGrant token exchange success', { j });
          await db.collection('users').doc(granteeToken).set({ alexaRefreshToken: j.refresh_token }, { merge:true });
        }
      }
    } catch(e) {
      log('ERROR', 'AcceptGrant processing failed', { msg: e.message, stack: e.stack });
    }
    return {
      event: {
    ...
    try {
      ...
      const endpoints = rooms.map(r => {
        ...
        return {
          ...
          friendlyName     : (r.name || 'Door Lock').replace(/door$/i, '').trim() + ' Door',
          description      : 'Locksure – Door status monitor (read-only)',
          displayCategories: ['SMARTLOCK'],
          capabilities     : buildEndpointCapabilities()
        };
      });
      return {
        event: {
  ...
  if ((namespace === 'Alexa.LockController' && name === 'ReportState') || 
    ...
    try {
      ...
      const prop = buildStateProperty(cur?.state);
      log('INFO', 'smartHome reportState mapped', { roomId, originalState: cur?.state, prop });
      return {
        context: {
          properties: [
            prop,
