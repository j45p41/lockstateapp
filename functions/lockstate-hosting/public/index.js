...
function mapLockState(state) {
  ...
  // 
  // =LOCKED, 2=UNLOCKED, 3=CLOSED (treat as LOCKED), 4=OPEN (treat as UNLOCKED)
  return {
...
exports.handler = async function(event, context) {
  ...
  if (path && path.replace(/\/+$/, '').endsWith('/alexaAuth') && httpMethod === 'GET') {
    ...
    // Use the incoming state parameter for firebaseUid
    const redirectState = queryStringParameters.state;
    console.log('Using dynamic UID for account linking:', redirectState);
    // Build the LWA URL
    const lwaAuthUrl = `${LWA_AUTH_URL}?client_id=${encodeURIComponent(queryStringParameters.client_id)}&scope=${encodeURIComponent(queryStringParameters.scope)}&response_type=${encodeURIComponent(queryStringParameters.response_type)}&redirect_uri=${encodeURIComponent(queryStringParameters.redirect_uri)}&state=${encodeURIComponent(redirectState)}`;
    console.log('Redirecting to LWA with params:', lwaAuthUrl);
    return {
      statusCode: 302,
      headers: {
        Location: lwaAuthUrl
  ...
  if (path && path.replace(/\/+$/, '').endsWith('/alexaToken') && httpMethod === 'POST') {
    console.log('=== /alexaToken called: OAuth is ENABLED ===');
    try {
      // Parse body (should be x-www-form-urlencoded)
      const params = typeof body === 'string' ? querystring.parse(body) : body;
      const { code, client_id, client_secret, redirect_uri, grant_type, state } = params;
      if (!code || !client_id || !client_secret || !redirect_uri || !grant_type || !state) {
        return {
          statusCode: 400,
          body: 'Missing required token parameters.'
        };
      }
      // Exchange code for access token with Amazon
      ...
      await storeAmazonUserIdMapping(state, amazonUserId);
      console.log('✅ Amazon user_id mapping stored for Firebase UID:', state);
      // Return token response to Alexa
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      console.error('❌ Error in /alexaToken:', error.message);
      return {
        statusCode: 500,
        body: 'Internal server error in /alexaToken.'
      };
    }
  }
